...
def logistic_map(x, r=3.99):  # 使用r值接近4以保持混沌状态
    return r * x * (1 - x)
class SSNE:
	...
	def __init__(self, args):
		...
		# 混沌初始化
		self.population = [logistic_map(np.random.rand()) for _ in range(self.population_size)]

	def selection_tournament(self, index_rank, num_offsprings, tournament_size):
		"""Conduct tournament selection with chaos
		...
		offsprings = []
		chaos_value = logistic_map(np.random.rand())  # 生成一个混沌值用于决策
		for i in range(num_offsprings):
			if chaos_value > 0.5:  # 根据混沌值调整竞赛大小
				tournament_size = max(2, int(tournament_size * chaos_value))
			else:
				tournament_size = max(2, int(tournament_size * (1 - chaos_value)))

			winner = np.min(np.random.randint(total_choices, size=tournament_size))
			offsprings.append(index_rank[winner])

		offsprings = list(set(offsprings))  # Find unique offsprings
		if len(offsprings) % 2 != 0:  # Ensure the number of offsprings is even
			offsprings.append(index_rank[winner])
		return offsprings
