...
def hadamard_gate(qubit):
    ...
    return (qubit + 1j * qubit) / np.sqrt(2)
...
class SSNE:
    def __init__(self, args):
        ...
        self.qubits = np.array([random.random() + 1j * random.random() for _ in range(self.population_size)])

    def measure_qubits(self):
        """Measure qubits to collapse their state to 0 or 1 based on probability amplitude"""
        probabilities = [abs(qubit.real) ** 2 for qubit in self.qubits]
        total_probability = sum(probabilities)
        if total_probability == 0:
            probabilities = [1.0 / len(self.qubits)] * len(
                self.qubits)  # Avoid division by zero by assigning equal probability
        else:
            probabilities = [p / total_probability for p in probabilities]  # Normalize probabilities

        # Ensure no zero probabilities to meet the requirement of numpy.random.choice
        probabilities = [max(p, 1e-10) for p in probabilities]
        measurements = [np.random.rand() < p for p in probabilities]  # Measure based on normalized probabilities
        return np.array(measurements, dtype=int)

    def selection_tournament(self, index_rank, num_offsprings, tournament_size):
        """Conduct tournament selection based on quantum measurements"""
        self.qubits = np.array([hadamard_gate(q) for q in self.qubits])  # Apply quantum gates before measurement
        measurements = self.measure_qubits()
        valid_probabilities = (measurements+0.0001) / np.sum(measurements+0.0001)

        total_choices = len(index_rank)
        offsprings = []
        for i in range(num_offsprings):
            if np.sum(valid_probabilities) == 0:
                participants = np.random.choice(index_rank,
                                                size=tournament_size)  # Fallback to uniform choice if all probabilities are zero
            else:
                participants = np.random.choice(index_rank, size=tournament_size, replace=False, p=valid_probabilities)
            winner = min(participants, key=lambda x: index_rank.index(x))
            offsprings.append(winner)

        offsprings = list(set(offsprings))  # Ensure unique offsprings
        if len(offsprings) % 2 != 0:  # Ensure even number of offsprings
            offsprings.append(random.choice(index_rank))
        return offsprings
    def list_argsort(self, seq):
        """Sort the list based on quantum measurements"""
        measurements = self.measure_qubits()
        return sorted(range(len(seq)), key=lambda x: measurements[x], reverse=True)
